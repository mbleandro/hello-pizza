...
```bash
dk bash  # starts bash inside "hello-pizza" container
...
* `dkup` uses docker-compose to start 3 containers: postgres, nginx, and hello-pizza.
...
* Once `dkup` is running, `dk <command>` will run `<command>` inside the `hello-pizza` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
* The hello-pizza container runs 3 services:
 * django on port 8000
...
```bash
mkvirtualenv hello-pizza -p python3  # creates a python3 virtualenv
